...
For an external role, the server accepts only the exact HTTPS source URL from the reviewed job and records a `handed_off` receipt. It does not send an external application, navigate a browser, or claim that the external site accepted a submission. WebMCP may prepare that handoff, but cannot bypass the human confirmation or open the link.
