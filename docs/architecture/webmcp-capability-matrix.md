...
| `registerTool()` is asynchronous                                     | One provider registers the complete 28-tool set before reporting ready                                                                                         | Partial failure cleans up the set and reports a recoverable unavailable state              | Agent activity changes from preparing to ready with the exact count  |
...
| Tool output should be concise and useful for the next action         | Action results are at most 1.5 KB; the advisory planner may use 2 KB, while the explicit 1–20 item application page is bounded to 16 KB                        | Size tests cover every workflow and representative success/error result                    | Search returns compact role facts and exact IDs                      |
...
All 28 tools are registered on every route. Nine are natural entry points:
