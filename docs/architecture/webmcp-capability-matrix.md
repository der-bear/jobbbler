...
| Tool output should be concise and useful for the next action         | Routine actions are at most 1.5 KB and the planner 2 KB; only exact-detail reads use larger named bounds: 16 KB for an application index, 20 KB for a role, and 64 KB for the final private application review | Size tests cover every workflow and representative success/error result                    | Search stays compact while review returns the exact values being approved |
...
- `plan_job_workflow` — optional, read-only, route-aware advice for six goals;
