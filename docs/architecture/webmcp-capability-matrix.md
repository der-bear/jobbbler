...
| Human interaction APIs and consent semantics are still evolving      | `requires_user_action` returns a server request ID plus bounded presentation facts or a compact reference to an exact visible owner review               | The server accepts only a decision bound to the live request and reviewed resource version | The agent asks once for an alert, assistance, and exact submission   |
...
6. `request_submission_review` freezes the exact application on the visible
   ...
   version, draft version, and expiry. Exact values do not enter the WebMCP
   result or external agent client.
