...
| Tool output should be concise and useful for the next action         | Results are JSON-serializable and bounded: 1.5 KB normally, 2 KB for the intent-rich plan, and 64 KB only for an exact application or alert review       | Size tests cover every workflow and representative success/error result                    | Search returns compact role facts and exact IDs                      |
...
| Human interaction APIs and consent semantics are still evolving      | `requires_user_action` returns a server request ID plus exact presentation facts for the external agent client                                           | The server accepts only a decision bound to the live request and reviewed resource version | The agent asks once for an alert, assistance, and exact submission   |
| Consent evidence is application responsibility                       | Jobbbler stores the reviewed purpose, policy or fields, notice version, decision channel, request evidence, and resource version before the exact action | Stale, replayed, mismatched, or declined decisions cannot activate or submit               | Present an exact alert or application review before its decision     |
