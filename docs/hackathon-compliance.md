...
| WebMCP-powered web app                                    | Imperative `document.modelContext.registerTool` integration with the same 24 tools on every route, shared UI state, activity feedback, and lifecycle tests         | Live tools discovered and called   |
...
| Free judging access through judging period                | Public search and a loginless private owner session; no payment or demo account is required                                                                        | Availability monitor active        |
...
| Human-controlled agent authority                          | Resource/action/expiry-bound delegations, request-bound decisions relayed from the external agent client, backend re-evaluation, revoke, and denied-retry evidence | Authorization suite passes         |
| Consent and privacy by design                             | Granular data grants, exact recipient/purpose/payload binding, clear withdrawal, secret/PII redaction, and public architecture                                     | Privacy journey passes             |
| Observable agent interaction                              | Redacted cursor activity, point-of-effect feedback, optional Supabase wake-up broadcasts, reconnect/refetch semantics, and polling fallback                        | Realtime contract suite passes     |
