...
| WebMCP-powered web app                                    | Imperative `document.modelContext.registerTool` integration with the same 28 tools on every route, shared UI state, activity feedback, and lifecycle tests | Repository verified; live browser check pending      |
