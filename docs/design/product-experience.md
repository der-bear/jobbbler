...
| Role `/jobs/:jobId`           | Understand one opportunity              | `get_job_details`, `get_job_application_capability` (whether Jobbbler may prepare it, which decisions stay human, or whether the person continues on the employer site), `compare_jobs`                                                                                 | Provenance, freshness, known limits, search-fit evidence when criteria exist, and a clear next action                                                         |
