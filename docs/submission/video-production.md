...
| 0:08–0:23 | A website the agent can understand | Send the request from Codex in ChatGPT Desktop, open Jobbbler in its built-in browser, and show the native Site tools menu discovering 29 actions from the live page. Give `search_jobs`, `get_job_details`, and `compare_jobs` one readable close-up.                                                                                                                                                               | “Jobbbler gives the browser agent a precise interface to the live website. No separate MCP server. No guessing which pixels to click.”                                                                                                                                                                 |
| 0:23–0:53 | Search becomes an outcome          | The agent runs the search. The visible URL, filters, count, and cards update. Activity records the completed call. It opens one full role, inspects evidence, compares two roles, and returns a concise recommendation in chat.                                                                                                                                                                                      | “Some tools return structured facts quietly. Others move the live page with the agent. It searches accepted filters, reads complete roles, compares the strongest matches, and keeps the result visible.”                                                                                              |
...
> you would shortlist. Then turn on Jobbbler email updates for that exact search
> every weekday at 08:00 Europe/Berlin, only when the matches change. Ask me only
> if you need a missing detail or my decision.
...
- Do not claim support for every current agent client. Show the verified client
  ...
  agents.
- The native Site tools menu belongs to the ChatGPT Desktop built-in browser
  running Codex. Keep Chrome's Model Context Tool Inspector out of the final
  film; its verification evidence belongs only in the technical documentation.
